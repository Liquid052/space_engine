...
pub(crate) struct Packer {
    ...
    pub general_transformers: Vec<Arc<dyn AttributeTransformer>>,
}

#[derive(Resource, Debug, Default, Clone)]
pub struct TaggedObjDB {
    map: Arc<RwLock<HashMap<String, Packer>>>,
    general_map: Arc<RwLock<HashMap<String, Arc<dyn AttributeTransformer>>>>,
}

impl TaggedObjDB {
    pub fn read(&self) -> Result<TaggedObjReader, TaggedObjError> {
        self.map.read()
            .map(|borrowed_map| TaggedObjReader {
                map: borrowed_map
                ,
                attribute_map: self.general_map.read().unwrap(),
            })
    ...
    pub(crate) fn register_general(&self, builder: impl AttributeTransformer + 'static) {
...
pub struct TaggedObjReader<'a> {
    ...
    attribute_map: RwLockReadGuard<'a, HashMap<String, Arc<dyn AttributeTransformer>>>,
}

impl<'a> TaggedObjReader<'a> {
    pub fn pack(&self, mut ent_transform: EntityTransformer) {
        self.attribute_map.iter().for_each(|(_, transformer)| {
            let attribute = transformer.attribute();
            let contains_attribute = ent_transform.main_entity_ref().get_ref::<Pack>()
                .unwrap()
                .supports(attribute.0);
        ...
        let Some(packer) = self.map.get(type_name) else {
            return;
        };
