use crate::prelude::{EntityTransformer, TaggedObjDB};
...
#[derive(Reflect, Clone, Component, Default, Debug)]
#[reflect(Component, MapEntities)]
pub struct Pack {
    type_name: Cow<'static, str>,
    tags:      HashMap<Cow<'static, str>, PackTag>,

    supports: HashMap<String, ()>,
}
...
impl Pack {
    ...
    pub fn with_support(mut self, name: impl Into<String>) -> Self {
        self.supports.insert(name.into(), ());

        self
    }
    pub fn supports(&self, path: &str) -> bool {
        self.supports.contains_key(path)
    }
