...
fn update(star: &CelestialBody, bodies: &mut Soa<CelestialBody>) {
...
fn space_simulation_benchmark(c: &mut Criterion) {
    let mut group = c.benchmark_group("space_simulation");

    // Benchmark with different numbers of bodies
    for n_bodies in [100, 1000, 10000].iter() {
        group.bench_function(format!("n_bodies_{}", n_bodies), |b| {
            b.iter_batched(
                || setup(*n_bodies),  // setup function
                |(star, mut bodies)| update(&star, &mut bodies), // benchmark function
...
criterion_group!(benches, space_simulation_benchmark);
